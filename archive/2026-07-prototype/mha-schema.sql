-- =============================================================================
-- MyHealthAngel Partner Compliance Portal — reference schema
-- Postgres 14+ / Supabase. Adapt names freely; the relationships and the
-- security policies are the part that matters.
--
-- The row-level security policies at the bottom enforce partner isolation at
-- the DATABASE level. That is deliberate: it means an application-layer bug
-- cannot leak one partner's data to another.
--
-- If you are NOT using Postgres RLS, you must enforce every policy below in
-- your API layer instead. See section 5 of the handoff document.
-- =============================================================================
...
create table users (
  ...
  password_hash text        not null,          -- bcrypt(cost>=12) or argon2id. NEVER plain text.
...
create table materials (
  id                     uuid primary key default gen_random_uuid(),
  smid                   text            not null,
  status                 material_status not null default 'Submitted', -- MHA's own status
...
create table material_status_history (
  id          uuid primary key default gen_random_uuid(),
  material_id uuid not null references materials(id) on delete cascade,
  org_id      uuid references organizations(id) on delete cascade, -- NULL = MHA-level change
  from_status text,
  to_status   text not null,
  changed_by  text not null,   -- display name, e.g. 'MHA Compliance' or 'Bright Path Marketing'
...
--    For non-Supabase Postgres, set a session GUC per request instead, e.g.
--       SET LOCAL app.user_id = '<uuid>';
--    and read it with current_setting('app.user_id', true)::uuid
-- =============================================================================

-- Portable version: your API sets this once per request/transaction.
--     SET LOCAL app.user_id = '<uuid of logged-in user>';
-- ON SUPABASE, replace the body with:  select auth.uid();
-- (auth.uid() is only resolvable where Supabase's auth schema exists, so it is
--  deliberately not referenced here — this file must run on plain Postgres too.)
...
--    Maps 1:1 to the numbered rules in section 5 of the handoff document.
-- =============================================================================
...
-- Rule 5: own history + MHA-level history; never another org's
...
-- =============================================================================
-- 11. SMOKE TEST — proves isolation before you build the UI
-- Run as two different users; test 1 and 2 MUST return zero rows.
-- =============================================================================
--
--   -- as Partner A:
--   SET LOCAL app.user_id = '<partner A user id>';
--
--   -- 1) must return 0 rows: a material shared only with Partner B
--   SELECT * FROM materials WHERE id = '<B-only material id>';
--
--   -- 2) must return 0 rows: Partner B's messages on a shared material
--   SELECT * FROM messages WHERE org_id = '<org B id>';
--
--   -- 3) must return only A's row
--   SELECT * FROM material_org_status;
--
--   -- 4) must fail: writing another org's status
--   INSERT INTO material_org_status(material_id,org_id,status)
--   VALUES ('<shared material>','<org B id>','Approved');
--
-- =============================================================================
